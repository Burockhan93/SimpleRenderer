...
//Matrix-vector conversion for homogenous coordinate
Vec3f m2v(Matrix m) {
    return Vec3f(m[0][0] / m[3][0], m[1][0] / m[3][0], m[2][0] / m[3][0]);
}
//Convert from vector to matrix for homogenous coordinate
Matrix v2m(Vec3f v) {
    Matrix m(4, 1);
    m[0][0] = v.x;
    m[1][0] = v.y;
...
int main(int argc, char** argv) {
...

    {
        ...
        for (int i = 0; i < model->nfaces(); i++) {
            ...
            Vec3f world_coords[3];
            for (int j = 0; j < 3; j++) {
                Vec3f v = model->vert(face[j]);//faces hold the id of vertices
                screen_coords[j] = m2v(Viewport * Projection * v2m(v));//convert it to screen coord
                world_coords[j] = v;//keep as world coord as well
            }
        ...
        image.flip_vertically(); 
        image.write_tga_file("Coloredhead.tga");
    }
